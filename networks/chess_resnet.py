...
class ChessResNet(nn.Module):
    ...
    def forward(self, s):
        s = s.view(-1, 12, self.board_x, self.board_y)
...
class NNetWrapper:
    ...
    def examples_to_tensors(self, examples):
        ...
        boards = torch.FloatTensor(np.array(boards))  # Shape: (N, 8, 8, 12) for Chess
        pis = torch.FloatTensor(np.array(pis))        # Shape: (N, 4672) for Chess
