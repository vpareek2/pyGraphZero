...
class Connect4GAT(nn.Module):
    def __init__(self, game, args):
        ...
        self.feature_encoder = nn.Sequential(
            nn.Linear(5, args.num_channels),  # 3 state features + 2 position features
        ...
        self.gat_layers = nn.ModuleList([
            Connect4GATLayer(
                ...
                args.num_channels,
                args.num_heads
            ) for i in range(4)  # Deeper network
        ])
        ...
        # Policy head focusing on valid moves
    ...
    def forward(self, s):
        ...
        # Global pooling for board state
        x = x.view(s.size(0), -1)

        # Policy and value predictions
        pi = self.policy_head(x)
        # Mask invalid moves (full columns)
        valid_moves = self.get_valid_moves(s)
        pi = pi.masked_fill(~valid_moves, float('-inf'))
    ...
    def get_valid_moves(self, board):
        # Check top row for full columns
        return board[:, 0, 0, :] == 0

    def _board_to_graph(self, s):
        # Ensure s is a 4D tensor (batch_size, 3, 6, 7)
        if s.dim() == 3:
            s = s.unsqueeze(0)

        batch_size, channels, height, width = s.shape
        assert channels == 3 and height == 6 and width == 7, "Input should be (batch_size, 3, 6, 7)"

        # Reshape s to (batch_size * num_nodes, 3)
        x = s.view(batch_size * self.num_nodes, 3)

        # Create edge index for Connect4 graph structure
        edge_index_single = []
        for i in range(self.board_x):
            for j in range(self.board_y):
                ...
                if j < self.board_y - 1:
                    edge_index_single.extend([[node, node + 1], [node + 1, node]])
                # Vertical connections
                if i < self.board_x - 1:
                    edge_index_single.extend([[node, node + self.board_y], [node + self.board_y, node]])
                # Diagonal connections (top-left to bottom-right)
                if i < self.board_x - 1 and j < self.board_y - 1:
                    edge_index_single.extend([[node, node + self.board_y + 1], [node + self.board_y + 1, node]])
                # Diagonal connections (top-right to bottom-left)
                if i < self.board_x - 1 and j > 0:
                    edge_index_single.extend([[node, node + self.board_y - 1], [node + self.board_y - 1, node]])
